...
function SignupContent() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        // Pass metadata so the DB trigger can create profile automatically
        data: {
          full_name: form.name.trim(),
          name: form.name.trim(),
          role,
          store_name: role === 'seller' ? form.storeName.trim() : null,
        },
    ...
    if (data.user) {
      ...
      // Only try manual insert as a backup — never block on failure.
      try {
        const username = form.name.trim().toLowerCase()
          .replace(/[^a-z0-9]/g, '') + '_' + Math.floor(Math.random() * 99999);
      ...
      } catch { /* Trigger handles this — ignore */ }

      // Create store for sellers
      if (role === 'seller') {
        try {
          const slug = form.storeName.trim().toLowerCase()
        ...
        } catch { /* Can set up store later */ }
      }

      // If session exists → auto signed in (email confirmation disabled)
      if (data.session) {
  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', textAlign: 'center' }}>
      <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px' }}>
        <Check size={28} style={{ color: '#4ade80' }} />
      </div>
      <h2 style={{ fontFamily: 'Georgia,serif', fontSize: '1.6rem', fontWeight: 300, color: '#fff', marginBottom: 10 }}>Almost there!</h2>
      <p style={{ fontSize: '0.88rem', color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, maxWidth: 300, marginBottom: 8 }}>
        Account created for <strong style={{ color: '#C8A96B' }}>{form.email}</strong>
      </p>
      <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.35)', lineHeight: 1.6, maxWidth: 300, marginBottom: 28 }}>
        Check your inbox for a confirmation email. If email confirmation is disabled, you can sign in directly.
      </p>
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0a', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ width: '100%', maxWidth: 380 }}>
          <h1 style={{ fontFamily: 'Georgia,serif', fontSize: '1.6rem', fontWeight: 300, color: '#fff', marginBottom: 6, textAlign: 'center' }}>Create account</h1>
          <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)', textAlign: 'center', marginBottom: 24 }}>Join the fashion community</p>

          {/* Google */}
          <button onClick={handleGoogle} disabled={googleLoading} style={{
            width: '100%', padding: '13px 16px', borderRadius: 14, marginBottom: 16,
            background: '#fff', border: 'none', cursor: googleLoading ? 'not-allowed' : 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
            opacity: googleLoading ? 0.75 : 1, boxShadow: '0 2px 12px rgba(0,0,0,0.3)',
          }}>
            {googleLoading
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.25)' }}>or sign up with email</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.08)' }} />
          </div>

          {/* Role */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
            {[{ key: 'buyer', icon: '🛍️', label: 'Shop', desc: 'Buy fashion' }, { key: 'seller', icon: '🏪', label: 'Sell', desc: 'List products' }].map(({ key, icon, label, desc }) => (
              <button key={key} type="button" onClick={() => setRole(key as 'buyer' | 'seller')} style={{ padding: '12px 10px', borderRadius: 14, border: `1px solid ${role === key ? 'rgba(200,169,107,0.5)' : 'rgba(255,255,255,0.1)'}`, background: role === key ? 'rgba(200,169,107,0.08)' : '#111', cursor: 'pointer', textAlign: 'center' }}>
          ...
          <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Full name" autoComplete="name" style={{ ...inp, paddingLeft: 40 }} />
            </div>
            <div style={{ position: 'relative' }}>
              ...
              <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="Email address" autoComplete="email" style={{ ...inp, paddingLeft: 40 }} />
            </div>
            <div style={{ position: 'relative' }}>
              ...
              <input type={showPw ? 'text' : 'password'} value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="Password (min 6 characters)" autoComplete="new-password" style={{ ...inp, paddingLeft: 40, paddingRight: 44 }} />
              <button type="button" onClick={() => setShowPw(!showPw)} style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', padding: 0 }}>
            ...
            {role === 'seller' && (
              <div style={{ position: 'relative' }}>
                ...
                <input type="text" value={form.storeName}
                  onChange={e => { const n = e.target.value; setForm(f => ({ ...f, storeName: n, storeSlug: n.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') })); }}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }} onClick={() => setAgreed(!agreed)}>
              <div style={{ width: 18, height: 18, borderRadius: 5, border: `1.5px solid ${agreed ? '#C8A96B' : 'rgba(255,255,255,0.2)'}`, background: agreed ? '#C8A96B' : 'none', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: 1 }}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              {loading ? <div style={{ width: 16, height: 16, border: '2px solid rgba(0,0,0,0.3)', borderTopColor: '#000', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} /> : <>Create Account <ArrowRight size={15} /></>}
            </button>
          </form>

          <p style={{ textAlign: 'center', fontSize: '0.85rem', color: 'rgba(255,255,255,0.4)', marginTop: 20 }}>
            Already have an account?{' '}
            <Link href={`/auth/login${redirect !== '/' ? `?redirect=${encodeURIComponent(redirect)}` : ''}`} style={{ color: '#C8A96B', textDecoration: 'none', fontWeight: 500 }}>Sign in</Link>
          </p>
        </div>
      </div>
